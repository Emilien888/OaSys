...
public class ServerSetup {
	...
	public ServerSetup() {
		...
		case "windows":
			...
			File users = new File(dir+ "\\server.users.json");
			try {
				if(admins.createNewFile()) {
					System.out.println(TextColor.TEXT_YELLOW + servInf + "Created admins file" + TextColor.TEXT_RESET);
				...
				if(config.createNewFile()) {
					System.out.println(TextColor.TEXT_YELLOW + servInf + "Created config file" + TextColor.TEXT_RESET);
				...
				if(users.createNewFile()) {
					...
					System.out.println(servWar + "file <server.users.json> already exists. Please check this is intentional");
				}
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		case "unix":
			...
			File users1 = new File(dir + "/server.users.json");
			try {
